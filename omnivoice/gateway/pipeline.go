...
func (p *Pipeline) sendGreeting(text string) error {
	...
	// Send audio
	p.session.logger.Info("sending greeting audio to Twilio", "bytes", len(audio))
	if err := p.sendAudioToTwilio(audio); err != nil {
		p.session.logger.Error("failed to send greeting audio to Twilio", "error", err)
...
func (p *Pipeline) processAudio(audio []byte) {
	...
	p.session.logger.Info("sending audio to Twilio", "bytes", len(audio))
	if err := p.sendAudioToTwilio(audio); err != nil {
...
// sendAudioToTwilio sends audio data to Twilio in chunks.
func (p *Pipeline) sendAudioToTwilio(audio []byte) error {
	...
	// Send in chunks (Twilio recommends ~20ms chunks for 8kHz = 160 samples = 160 bytes for μ-law)
