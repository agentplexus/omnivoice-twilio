...
// NewGatewayProvider creates a Twilio gateway from registry config.
func NewGatewayProvider(cfg registry.ProviderConfig) (registry.Gateway, error) {
	config := Config{
		AccountSID: getExtString(cfg.Extensions, "accountSID"),
		AuthToken:  getExtString(cfg.Extensions, "authToken"),
	}
	...
	if config.AccountSID == "" {
		return nil, fmt.Errorf("twilio gateway: accountSID is required")
	}
	if config.AuthToken == "" {
		return nil, fmt.Errorf("twilio gateway: authToken is required")
	}
